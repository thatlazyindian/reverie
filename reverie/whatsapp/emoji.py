...
def prepare_data(df):
    ...
    * Different_Emojis - Number of unique emojis used in a message
    ...
    df['Emoji'] = df.apply(lambda row: extract_emojis(str(row.chat)), 
                                       axis = 1)
    ...
    df['Different_Emojis'] = df.apply(lambda row: re.findall(pattern, str(row.chat)), 
                                                  axis=1)
...
def plot_corr_matrix(df, user, list_of_words, counts):
    ...
    for emoji_str in list_of_words:
        df[emoji_str] = df.apply(lambda x: x.chat.count(emoji_str), 1)
